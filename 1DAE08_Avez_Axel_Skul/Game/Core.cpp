...
void Core::RunGameLoop()
{
	...
	SpriteRenderer renderer = SpriteRenderer(pGame->GetSprites());
	OverlapTester overlapTester = OverlapTester(pGame->GetSprites());
	PhysicsEngine physicsEngine = PhysicsEngine(pGame->GetSprites());
	SpriteAnimator animator = SpriteAnimator(pGame->GetSprites());
	...
	while (!HandleSDLEvents(pGame)) {
		...
		if (isTimeToRender) {
			//std::cout << "frame render after: " << deltaTime << "ms " << 1.0f/deltaTime << "fps" << std::endl;;
			...
			//check for overlaps (fires overlap events inside sprites)
			overlapTester.TestOverlap();
			//update animations
